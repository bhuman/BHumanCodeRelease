...
{
  DEBUG_DRAWING("representation:GoaliPose:coverage", "drawingOnField")
  {
    if(Blackboard::getInstance().exists("BallModel") && Blackboard::getInstance().exists("RobotPose") && Blackboard::getInstance().exists("FieldDimensions"))
    {
      ...
      auto drawOffset = [&](const float range, [[maybe_unused]] const ColorRGBA color)
      {
        ...
        POLYGON("representation:GoaliPose:coverage", 4, points, 10, Drawings::noPen, color, Drawings::solidBrush, color);
      };
      ...
      LINE("representation:GoaliPose:coverage", this->goaliePoseField.translation.x(), this->goaliePoseField.translation.y(), gloBallPos.x(), gloBallPos.y(), 50.f, Drawings::dashedPen, ColorRGBA::black);
    }
  }
}
...
void GoaliePose::verify() const
{
  ...
  ASSERT(goaliePoseField.translation.x() > -6500.f);
