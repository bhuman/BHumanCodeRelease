...
STREAMABLE_WITH_BASE(FieldFeature, Pose2f,
{
  ...
  {
    RobotPoseToFF(const Pose2f& pose1, const Pose2f& pose2) : pos1(pose1) COMMA pos2(pose2) {};
    RobotPoseToFF(const Pose2f& pose) : pos1(pose) COMMA pos2(pose.dotMirror()) {},
  ...
  FieldFeature(const Pose2f& pose) : Pose2f(pose) COMMA isValid(true){};
   ...
   * that are inside the by parameter given area around the field features
   * position.
   *
   * @param searchXRadius, measurement of the area to both sides of the x-achis 
   *    according to the features pose
   * @param searchYRadius, measurement of the area to both sides of the y-achis 
   *    according to the features pose
   * @param theFieldLines, the representations it says
   * @param offset (optional), the offset the search area to the features position
   * @param allowedLines (optional), the amound of unmarked linies that is allowed 
   ...
   * Because on the field all features are dot mirrored this function will
