...
namespace Drawings
{
  ...
  enum ShapeType
  {
    ...
    thread, gridMono, gridRGBA, gridRectangleRGBA
  };
 ...
 * @param cellsX The number of cells on the x axis
 * @param cellsY The number of cells on the y axis
 ...
 * @param cellsX The number of cells on the x axis
 * @param cellsY The number of cells on the y axis
 * @param cells An array (of RGBA) containing the colors of the cells
 */
 ...
 * @param cells An array (of RGBA) containing the colors of the cells
 */
#define GRID_RECTANGLE_RGBA(id, x, y, cellWidth, cellHeight, cellsX, cellsY, cells) \
  do \
    COMPLEX_DRAWING(id) \
    { \
      const int _cellsX = static_cast<int>(cellsX); \
