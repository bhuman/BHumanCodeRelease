...
void LogPlayer::updateIndices()
{
  ...
  for(auto i = begin(); i != end(); ++i)
  {
    ...
    switch(id(message))
    {
      ...
      case idAnnotation:
      {
        Annotation& annotation = annotationsPerThread[currentThread].emplace_back();
        (*i).bin() >> annotation;
...
bool LogPlayer::readIndices(In& stream, size_t& usedSize)
{
  ...
  for(unsigned i = 0; i < size; ++i)
  {
    ...
    std::vector<Annotation>& annotations = annotationsPerThread[threadName];
    for(unsigned j = 0; j < annotationsSize; ++j)
    {
      Annotation& annotation = annotations.emplace_back();
      stream >> annotation.number >> annotation.frame >> annotation.name >> annotation.description;
    }
  }
...
void LogPlayer::filterFrames(const std::function<bool(const_iterator)>& keep)
{
  bool keepFrame = false;
  size_t sizeBeforeFrame = 0;
  filter([&](const_iterator i) -> bool
  {
    switch(id(*i))
    {
      case idFrameBegin:
        keepFrame = false;
        sizeBeforeFrame = size();
        break;
      case idFrameFinished:
        if(!keepFrame)
        {
          resize(sizeBeforeFrame);
          return false;
        }
        break;
      default:
...
const std::unordered_map<std::string, std::vector<Annotation>>& LogPlayer::annotations() const
{
  if(sizeWhenIndexWasComputed != size())
    const_cast<LogPlayer*>(this)->updateIndices();
