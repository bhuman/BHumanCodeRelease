  ...
  if(isDone)
  {
    ...
    switch(motionRequest.motion)
    {
      ...
      case MotionRequest::walkToBallAndKick:
        requestWalkToBallAndKick(motionRequest.ballEstimate, motionRequest.targetDirection, motionRequest.kickType, motionRequest.kickLength,
      ...
      case MotionRequest::dribble:
        ...
        break;
      case MotionRequest::dive:
        requestDive(motionRequest.diveRequest);
        break;
      case MotionRequest::special:
        requestSpecial(motionRequest.specialRequest);
        break;
      default:
...
void SimulatedRobot2D::requestWalkToBallAndKick(const BallState& ballEstimate, Angle targetDirection, KickInfo::KickType kickType, float kickLength, bool alignPrecisely, const Pose2f& walkSpeed, const MotionRequest::ObstacleAvoidance& obstacleAvoidance)
{
  ...
  if(isInPositionForKick)
  {
    ...
    currentPhase.kickPower = KickLengthConverter::kickLengthToPower(kickType, kickLength, targetDirection, kickInfo);
...
void SimulatedRobot2D::requestDive(MotionRequest::Dive::Request diveRequest)
{
  currentPhase.motion = MotionPhase::keyframeMotion;
  currentPhase.keyframeMotionRequest = KeyframeMotionRequest::fromDiveRequest(diveRequest);
  switch(diveRequest)
  {
...
void SimulatedRobot2D::requestSpecial(MotionRequest::Special::Request specialRequest)
{
  currentPhase.motion = MotionPhase::keyframeMotion;
  currentPhase.keyframeMotionRequest = KeyframeMotionRequest::fromSpecialRequest(specialRequest);
  currentPhase.duration = 0.f;
}
  ...
  if(currentPhase.kick)
  {
    if(currentPhase.t >= currentPhase.kick->impactT && !currentPhase.performedAction)
    {
      ...
      if(ball)
      {
        ...
        // Inform the GameController about the contact.
