...
void SoundPlayer::main()
{
  ...
  [[maybe_unused]] const HRESULT& hr = CoCreateInstance(CLSID_SpVoice, NULL, CLSCTX_ALL, IID_ISpVoice, reinterpret_cast<void**>(& pVoice));
  ASSERT(SUCCEEDED(hr));
...
void SoundPlayer::sayDirect(const std::string& text)
{
  ...
  // Speak async
  [[maybe_unused]] const HRESULT& hr = pVoice->Speak(pcwstr, SPF_ASYNC, NULL);
  ASSERT(SUCCEEDED(hr));
  delete[] pcwstr;
