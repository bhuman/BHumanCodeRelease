 ...
 * @author Colin Graf
 * Addition for moving Balls(Percepts).
...
BallLocator::BallLocator() : timeBallWasBeenSeenInLowerCameraImage(0), timeWhenBallFirstDisappeared(0), ballDisappeared(true)
{
  init();
}
...
void BallLocator::update(BallModel& ballModel)
{
  ...
  DECLARE_DEBUG_DRAWING("module:BallLocatorPerceptRedeemer:redemption", "drawingOnField");
  ...
  // checks cross-checks percept-positions with previous percept-positions to detect percepts that fit into a straight movement-pattern
  movingBallPerceptRedeemer.update(theBallPercept, theFrameInfo.time, theOdometryData);
  ...
  bool ballIsInImage = theBallPercept.status == BallPercept::seen ||
                       (theBallPercept.status == BallPercept::guessed && ballModel.estimate.velocity.norm() != 0.f &&
                        theWorldModelPrediction.ballIsValid &&
                        (theWorldModelPrediction.ballPosition - theBallPercept.positionOnField).norm() < guessedRollingBallAcceptanceDistance);
  if(!ballIsInImage && movingBallPerceptRedeemer.plausiblePercept)
  {
    ANNOTATION("MovingBallPerceptRedeemer", "Percept exonerated.");
  }
  ballIsInImage = ballIsInImage || movingBallPerceptRedeemer.plausiblePercept; //-> also accept plausible movements, "see movingBallPerceptRedeemer.update"
  ...
  if(bestState != nullptr && theMotionInfo.isMotionStable)
...
void BallLocator::motionUpdate(BallModel& ballModel)
{
  ...
  for(BallHypothesis* state = states, *end = states + stateCount; state < end; ++state)
  {
    state->motionUpdate(ballModel, movingA, movingATransposed, movingOdometryTranslation,
                        ...
                        fixedOdometryRotationDeviationRotation, theBallSpecification, deltaTime);
  }
}

void BallLocator::collisionForFootCalculation(Vector2f& assumedLastBallPosition, Vector2f& assumedBallOffset, const Vector2f& ballPosition, const Vector2f& lastBallPosition, const Vector2f& footCenter, const Vector2f& lastFootCenter, const float radius, float& collisionFactor, bool& collision)
{
  assumedLastBallPosition = lastBallPosition + (footCenter - lastFootCenter);
  assumedBallOffset = ballPosition - assumedLastBallPosition;
  collision = BallLocatorTools::getSmallestLineWithCircleIntersectionFactor(assumedLastBallPosition, assumedBallOffset, lastFootCenter, radius, collisionFactor);
...
void BallLocator::handleCollisionWithFeet(const Vector2f& leftFootCenter, const Vector2f& rightFootCenter)
{
  ...
  collisionForFootCalculation(leftAssumedLastBallPosition, leftAssumedBallOffset, ballPosition, lastBallPosition, leftFootCenter, lastLeftFootCenter, assumedRadius, leftCollisionFactor, leftCollision);

  // detect collision with the right foot
  collisionForFootCalculation(rightAssumedLastBallPosition, rightAssumedBallOffset, ballPosition, lastBallPosition, rightFootCenter, lastRightFootCenter, assumedRadius, rightCollisionFactor, rightCollision);
...
void BallLocator::normalizeWeights(BallHypothesis*& bestState, BallHypothesis*& worstStationaryState, BallHypothesis*& worstMovingState)
{
  bestState = nullptr;
...
void BallLocator::createNewStates(const Vector2f& ballPercept, const float ballPerceptRadius, const Matrix2f& ballPerceptCov, BallHypothesis* worstStationaryState, BallHypothesis* worstMovingState)
{
...
bool BallLocator::ballCanBeExcludedByLocalization()
{
...
bool BallLocator::ballIsAtBorderAndCloseToTeammate()
{
  ...
  if(x + r > theCameraInfo.width || x - r < 0 || y - r < 0)
  {
    ...
    for(auto const& teammate : theTeamData.teammates)
    {
      if(teammate.status == Teammate::PLAYING || teammate.status == Teammate::FALLEN)
      {
        ...
        {
          ANNOTATION("BallLocator", "Excluded border ball close to teammate.");
          return true;
        }
      }
    }
