/**
 * @file BallPerceptFilterRollingBallChallenge.cpp
 *
 ...
 * Furthermore, it contains small modifications for the 2025 KICKin’ & Rollin’ Challenge.
 *
 * @author Tim Laue
 */

#include "BallPerceptFilterRollingBallChallenge.h"
#include "Debugging/Annotation.h"
#include "Debugging/Plot.h"
#include "Tools/Math/Projection.h"
#include "Tools/Modeling/BallLocatorTools.h"

MAKE_MODULE(BallPerceptFilterRollingBallChallenge);

BallPerceptFilterRollingBallChallenge::BallPerceptFilterRollingBallChallenge() : timeBallWasBeenSeenInLowerCameraImage(0),
                                         timeWhenLastKickWasExecuted(0), timeOfLastFilteredPercept(0), shakiness(0.f)
{
}

void BallPerceptFilterRollingBallChallenge::update(FilteredBallPercepts& filteredBallPercepts)
{
  // Declare plots as drawing code is not always executed:
  DECLARE_PLOT("module:BallPerceptFilterRollingBallChallenge:angleBasedDistance");
  DECLARE_PLOT("module:BallPerceptFilterRollingBallChallenge:sizeBasedDistance");
  DECLARE_PLOT("module:BallPerceptFilterRollingBallChallenge:combinedDistance");
  ...
  PLOT("module:BallPerceptFilterRollingBallChallenge:shakiness", shakiness);
  ...
  // ***** Workaround for ball on a ramp: Use ball percept position based on perceived size, if it appears to be closer than
  // the normal projection-based position (which is too far away, as long as the ball is on the ramp).
  const float distanceToBallProjectionBased = perceivedBallPosition.norm();
  const float distanceToBallSizeBased = Projection::getDistanceBySize(theCameraInfo, theBallSpecification.radius, theBallPercept.radiusInImage,
                                                                      theBallPercept.positionInImage.x(), theBallPercept.positionInImage.y());
  if(distanceToBallSizeBased < distanceToBallProjectionBased)
    perceivedBallPosition.normalize(distanceToBallSizeBased);
  // End of workaround *****
...
bool BallPerceptFilterRollingBallChallenge::perceptCanBeExcludedByLocalization()
{
  ...
  if(!theFieldDimensions.isInsideField(ballOnField))
  {
    ...
    {
      ANNOTATION("BallPerceptFilterRollingBallChallenge", "Excluded ball outside the field.");
      return true;
    }
  }
  return false;
...
bool BallPerceptFilterRollingBallChallenge::perceptIsInsideTeammateAndCanBeExcludedByTeamBall()
{
    ...
    if((teammate.getEstimatedPosition(theFrameInfo.time) - ballOnField).norm() < robotBanRadius)
    {
         ...
         teammate.theBallModel.estimate.position.norm() > robotBanRadius)
      {
        ...
        if(theTeamBallModel.isValid && (theTeamBallModel.position - ballOnField).norm() > 1500)
        {
          ...
          if(theBallPercept.status == BallPercept::seen)
          {
            ANNOTATION("BallPerceptFilterRollingBallChallenge", "Excluded Hackbaellchen!");
            //OUTPUT_TEXT("BallPerceptFilterRollingBallChallenge : Excluded Hackbaellchen!");
          }
          return true;
        }
      }
...
bool BallPerceptFilterRollingBallChallenge::ballPerceptIntersectsObstaclesPercept()
{
...
bool BallPerceptFilterRollingBallChallenge::perceptIsInOtherHalf()
{
  const Vector2f ballOnField = theWorldModelPrediction.robotPose * theBallPercept.positionOnField;
...
bool BallPerceptFilterRollingBallChallenge::verifySeenBall()
{
...
bool BallPerceptFilterRollingBallChallenge::currentlyPerceivedGuessedBallIsCloseToPreviouslyPerceivedSeenBalls()
{
...
bool BallPerceptFilterRollingBallChallenge::robotRecentlyKickedAndThereAreGuessedBallsRollingAway()
{
  // Method should not have been called in this case:
  if(theBallPercept.status != BallPercept::guessed)
    return false;
  ...
  ANNOTATION("BallPerceptFilterRollingBallChallenge", "Found a sequence of " << lastIndex + 1 << " balls with an increasing distance after a kick!");
  return true;
}

bool BallPerceptFilterRollingBallChallenge::perceptsAreOnALineThatIsCompatibleToARollingBall()
{
  // Method should not have been called in this case:
  if(theBallPercept.status != BallPercept::guessed)
    return false;
  ...
  {
    ANNOTATION("BallPerceptFilterRollingBallChallenge", "Found a sequence that indicates a rolling ball: " << stdDevMoving << " < " << stdDevStatic << "  with " << speed << "mm/s!");
...
float BallPerceptFilterRollingBallChallenge::computeStdDevOfStaticBallHypothesis(unsigned indexOfOldestObservation)
{
...
float BallPerceptFilterRollingBallChallenge::computeStdDevOfMovingBallHypothesis(unsigned indexOfOldestObservation, Vector2f& endVelocity)
{
...
void BallPerceptFilterRollingBallChallenge::plotAndDraw()
{
  if(theBallPercept.status == BallPercept::seen && theCameraInfo.camera == CameraInfo::upper)
  {
    ...
    PLOT("module:BallPerceptFilterRollingBallChallenge:angleBasedDistance", angleBasedDistance);
    ...
    PLOT("module:BallPerceptFilterRollingBallChallenge:sizeBasedDistance", sizeBasedDistance);

    const float usedShakiness = shakiness > 0.5f ? 0.5f : shakiness;
    float sizeFactor = mapToRange(usedShakiness, 0.f, 0.5f, 0.f, 1.f);
    float angleFactor = 1.f - sizeFactor;
    const float combinedDistance = sizeFactor * sizeBasedDistance + angleFactor * angleBasedDistance;
    PLOT("module:BallPerceptFilterRollingBallChallenge:combinedDistance", combinedDistance);
  }
}
