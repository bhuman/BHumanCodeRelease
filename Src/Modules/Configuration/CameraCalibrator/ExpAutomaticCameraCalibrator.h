/**
 * @file ExpAutomaticCameraCalibrator.h
 *
...
MODULE(ExpAutomaticCameraCalibrator,
{,
  REQUIRES(CameraInfo),
  ...
  {,
    ...
    (Angle)(1_deg) maxOpeningAngleAdjustment, /**< Optimizing the opening angles shall not deviate too much from the default. */
  }),
});

class ExpAutomaticCameraCalibrator : public ExpAutomaticCameraCalibratorBase
{
public:
  /** Constructor. */
  ExpAutomaticCameraCalibrator();
...
private:
  ...
  {
    /** Constructor. */
    Sample(SampleType sampleType, const ExpAutomaticCameraCalibrator& calibrator, const TorsoMatrix& torsoMatrix, float headYaw, float headPitch, const CameraInfo& cameraInfo, const ImageCoordinateSystem& coordSys) :
     ...
     */
    float computeError(const CameraCalibration& cameraCalibration, const CameraInfo& cameraInfo) const;
     ...
     * @return The error (usually between 0 and 1).
     */
    virtual float computeError(const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo) const = 0;

    const SampleType sampleType;
    const ExpAutomaticCameraCalibrator& calibrator; /**< The owning module. */
  ...
  struct SampleConfiguration
  {
    ...
    /**
     * Checks if this configuration wants to recorded a sample of a given type.
     * @param sampleType The sample type to check.
     * @return See above.
     */
    bool containsSampleType(SampleType sampleType) const;
  };

  struct CornerAngleSample : Sample
  {
    CornerAngleSample(const ExpAutomaticCameraCalibrator& calibrator, const TorsoMatrix& torsoMatrix, float headYaw, float headPitch,
                      const CameraInfo& cameraInfo, const ImageCoordinateSystem& coordSys, const CorrectedLine& cLine1, const CorrectedLine& cLine2) :
      Sample(cornerAngle, calibrator, torsoMatrix, headYaw, headPitch, cameraInfo, coordSys), cLine1(cLine1), cLine2(cLine2) {}
    float computeError(const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo) const override;
  ...
  {
    ParallelAngleSample(const ExpAutomaticCameraCalibrator& calibrator, const TorsoMatrix& torsoMatrix, float headYaw, float headPitch,
                        const CameraInfo& cameraInfo, const ImageCoordinateSystem& coordSys, const CorrectedLine& cLine1, const CorrectedLine& cLine2) :
      Sample(parallelAngle, calibrator, torsoMatrix, headYaw, headPitch, cameraInfo, coordSys), cLine1(cLine1), cLine2(cLine2) {}
    float computeError(const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo) const override;
  ...
  {
    ParallelLinesDistanceSample(const ExpAutomaticCameraCalibrator& calibrator, const TorsoMatrix& torsoMatrix, float headYaw, float headPitch,
                                const CameraInfo& cameraInfo, const ImageCoordinateSystem& coordSys, const CorrectedLine& cLine1, const CorrectedLine& cLine2) :
      Sample(parallelLinesDistance, calibrator, torsoMatrix, headYaw, headPitch, cameraInfo, coordSys), cLine1(cLine1), cLine2(cLine2) {}
    float computeError(const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo) const override;
    mutable CorrectedLine cLine1, cLine2; /**< The corrected ground line and front goal line. */
  };
  ...
  {
    GoalAreaDistanceSample(const ExpAutomaticCameraCalibrator& calibrator, const TorsoMatrix& torsoMatrix, float headYaw, float headPitch,
      ...
      Sample(goalAreaDistance, calibrator, torsoMatrix, headYaw, headPitch, cameraInfo, coordSys), penaltyMarkInImage(penaltyMarkInImage), cLine(cLine) {}
    float computeError(const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo) const override;
  ...
  {
    GroundLineDistanceSample(const ExpAutomaticCameraCalibrator& calibrator, const TorsoMatrix& torsoMatrix, float headYaw, float headPitch,
                             const CameraInfo& cameraInfo, const ImageCoordinateSystem& coordSys, const Vector2i& penaltyMarkInImage, const CorrectedLine cLine) :
      Sample(groundLineDistance, calibrator, torsoMatrix, headYaw, headPitch, cameraInfo, coordSys), penaltyMarkInImage(penaltyMarkInImage), cLine(cLine) {}
    float computeError(const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo) const override;
    Vector2i penaltyMarkInImage; /**< The penalty mark in the image. */
  ...
  {
    /** Constructor. */
    Functor(ExpAutomaticCameraCalibrator& calibrator) : calibrator(calibrator) {}
    ...
    ExpAutomaticCameraCalibrator& calibrator; /**< The owning module. */
  };
  ...
  /**
   * Updates the CameraIntrinsics.
   * @param cameraIntrinsics The representation updated.
   */
  void update(CameraIntrinsics& cameraIntrinsics) override;
   ...
   */
  Parameters pack(const CameraCalibration& cameraCalibration, const CameraIntrinsics& cameraIntrinsics) const;
   ...
   * @param cameraCalibration The camera calibration.
   */
  void unpack(const Parameters& params, CameraCalibration& cameraCalibration, CameraIntrinsics& cameraIntrinsics) const;

  void updateSampleConfiguration();

  bool isSampleConfigurationRecorded(const SampleConfigId& index) const;
  void recordSampleConfiguration(const SampleConfiguration& sampleConfiguration, std::vector<std::unique_ptr<Sample>>& samples);
  ...
  std::set<SampleConfigId> recordedSampleConfigurations; /**< The indices of sample configurations that were actually recorded. */
  std::vector<std::unique_ptr<Sample>> samples; /**< The array of recorded samples. */

  CameraCalibration nextCameraCalibration; /**< The camera calibration which is set as next camera calibration by this module. */
  CameraIntrinsics nextCameraIntrinsics; /**< The camera calibration which is set as next camera calibration by this module. */
  CameraIntrinsics originalOpeningAngles;
