...
public:
  ...
  /*! Note: At the moment the user has to take care to delete \c filename the parameters have been changed.
      If \c filename is \c NULL, the table is neither loaded nor saved.
   */
  void loadOrCreate(const TriangleMesh& object, const Eigen::AlignedBox3d& viewpointRange, double spacing, const char* filename = NULL);
