...
#include "isometryWithResponse.h"
...
/*! The detector detects objects given by a \c TriangleMesh in a
    ...
    The detector uses the contrast-normalized-Sobel (CNS, \c CNSImage)
...
public:
  ...
  ObjectCNSStereoDetector(const TriangleMesh& object, const CameraModelOpenCV& camera, Eigen::AlignedBox3d& viewpointRange, double spacing, const SearchSpecification& spec = SearchSpecification(), ParametricLaplacian distribution = ParametricLaplacian(), const char* filenameForLutRasterizer = NULL)
   ...
   */
  void create(const TriangleMesh& object, const CameraModelOpenCV& camera, Eigen::AlignedBox3d& viewpointRange, double spacing,
              const SearchSpecification& spec = SearchSpecification(), const ParametricLaplacian& distribution = ParametricLaplacian(),
              const char* filenameForLutRasterizer = NULL);
   ...
   */
  double response(const CNSImage& cns, const Eigen::Isometry3d& object2World) const;
  double response(const CNSImage& cns, const CodedContour& ct) const;

  //! Computes the response for \c object2World and stores it in \c object2World.response
  void computeResponse(const CNSImage& cns, IsometryWithResponse& object2World) const
  ...
  /*! Evaluates the response of this object in the pose \c object2World, \c object2World*newObject2Object
    ...
    the discrete maximum was at the border (\c false) and no interpolation could be done.
      ...
      a subpixel interpolated maximum.
   */
  bool refine1Dimension(const CNSImage& cns,
                        IsometryWithResponse& object2World, double stepInPixel, int dimension) const;
  ...
  /*! Calls \c refine1Dimension \c ctr times in each directions \c DIM_TRANS_VIEWING, DIM_ROT_X/Y/Z
      ...
      a subpixel interpolated maximum.
   */
  void refine(const CNSImage& cns, IsometryWithResponse& object2World, int iterationCtr = 1) const;
  ...
  /*! \c max must be a discrete maximum, and \c response a pointer
      into that entry of a unsigned char [][16][16] array that
