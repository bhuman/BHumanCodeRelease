...
void  ObjectCNSStereoDetector::create(const TriangleMesh& object, const CameraModelOpenCV& camera,
                                      Eigen::AlignedBox3d& viewpointRange, double spacing,
...
{
  double stepInPixel = 3;
  for(int i = 0; i < iterationCtr; i++)
  {
    refine1Dimension(cns, object2World, stepInPixel, DIM_TRANS_VIEWING);
    if(!object.isRotationalSymmetric)
    {
      refine1Dimension(cns, object2World, stepInPixel, DIM_ROT_X);
      refine1Dimension(cns, object2World, stepInPixel, DIM_ROT_Y);
      if(!object.isRotationalSymmetricZ)
        refine1Dimension(cns, object2World, stepInPixel, DIM_ROT_Z);
    }
  }
}
