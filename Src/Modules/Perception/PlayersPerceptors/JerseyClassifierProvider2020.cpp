/**
 * @file JerseyClassifierProvider2020.cpp
 *
...
#include "JerseyClassifierProvider2020.h"
#include "Debugging/DebugDrawings.h"
#include "Tools/Math/Transformation.h"
#include <algorithm>
#include <cmath>

MAKE_MODULE(JerseyClassifierProvider2020, perception);

void JerseyClassifierProvider2020::update(JerseyClassifier& jerseyClassifier)
{
  DECLARE_DEBUG_DRAWING("module:JerseyClassifierProvider2020:jerseyRange", "drawingOnImage");
  DECLARE_DEBUG_DRAWING("module:JerseyClassifierProvider2020:jersey", "drawingOnImage");
  DECLARE_DEBUG_DRAWING("module:JerseyClassifierProvider2020:jerseyWeights", "drawingOnImage");
  DECLARE_DEBUG_DRAWING("module:JerseyClassifierProvider2020:jerseyClassification", "drawingOnImage");
...
void JerseyClassifierProvider2020::detectJersey(const ObstaclesImagePercept::Obstacle& obstacleInImage, ObstaclesFieldPercept::Obstacle& obstacleOnField) const
{
  ...
  RECTANGLE("module:JerseyClassifierProvider2020:jerseyClassification", obstacleInImage.left, obstacleInImage.top, obstacleInImage.right, obstacleInImage.bottom, 3, Drawings::solidPen, ColorRGBA::violet);
  DRAW_TEXT("module:JerseyClassifierProvider2020:jerseyClassification", obstacleInImage.left, obstacleInImage.top - 24, 10, ColorRGBA::black, "distance: " << distance);
                                     ...
                                     theCameraMatrix, theCameraInfo, upperInImage))
  {
    ...
    if(lowerInImage.y() >= jerseyMinYSamples)
    {
      ...
      if(ySteps < jerseyMinYSamples)
      {
        ...
        DRAW_TEXT("module:JerseyClassifierProvider2020:jerseyClassification", obstacleInImage.left, obstacleInImage.top + 12, 10, ColorRGBA::black, "class: unknown");
        return;
      }
      ...
      if(theGameState.ownTeam.color == GameState::Team::Color::black || theGameState.opponentTeam.color == GameState::Team::Color::black ||
         theGameState.ownTeam.color == GameState::Team::Color::gray || theGameState.opponentTeam.color == GameState::Team::Color::gray ||
         theGameState.ownTeam.color == GameState::Team::Color::white || theGameState.opponentTeam.color == GameState::Team::Color::white)
      {
      ...
      const std::function<bool(int, int)>& isOwn = getPixelClassifier(theGameState.ownTeam.color, theGameState.opponentTeam.color, maxBrightness);
      const std::function<bool(int, int)>& isOpponent = getPixelClassifier(theGameState.opponentTeam.color, theGameState.ownTeam.color, maxBrightness);
          ...
          y += yStep, left = std::max(left + xyStep, 0.f), right = std::min(right + xyStep, static_cast<float>(theCameraInfo.width)))
      {
        ...
        for(float x = left; x < right; x += xStep)
        {
          ...
          DOT("module:JerseyClassifierProvider2020:jerseyWeights", static_cast<int>(x), static_cast<int>(y),
          ...
          {
            ownPixels += weight;
            DOT("module:JerseyClassifierProvider2020:jersey", static_cast<int>(x), static_cast<int>(y), ColorRGBA::yellow, ColorRGBA::yellow);
          }
          else if(isOpponent(static_cast<int>(x), static_cast<int>(y)))
          {
            opponentPixels += weight;
            DOT("module:JerseyClassifierProvider2020:jersey", static_cast<int>(x), static_cast<int>(y), ColorRGBA::blue, ColorRGBA::blue);
          }
          else
            DOT("module:JerseyClassifierProvider2020:jersey", static_cast<int>(x), static_cast<int>(y), ColorRGBA::green, ColorRGBA::green);
        }
      }
      // threshold to counter white robot parts or green field background being classified as opponent jersey
      if((theGameState.opponentTeam.color == GameState::Team::Color::white || theGameState.opponentTeam.color == GameState::Team::Color::green)
         && theGameState.ownTeam.color != GameState::Team::Color::white && theGameState.ownTeam.color != GameState::Team::Color::green)
      {
        const float threshold = 1.5f * ownPixels + (examinedPixels) / 5;
        opponentPixels = opponentPixels <= threshold ? 0 : opponentPixels - threshold;
      }
      DRAW_TEXT("module:JerseyClassifierProvider2020:jerseyClassification", obstacleInImage.left, obstacleInImage.top - 14, 10, ColorRGBA::black, "ownPlayer: " << ownPixels);
      DRAW_TEXT("module:JerseyClassifierProvider2020:jerseyClassification", obstacleInImage.left, obstacleInImage.top - 4, 10, ColorRGBA::black, "opponent: " << opponentPixels);
             ...
             || opponentPixels > (ownPixels + opponentPixels) * minJerseyRatio))
      {
        if(ownPixels > opponentPixels)
        {
          ...
          DRAW_TEXT("module:JerseyClassifierProvider2020:jerseyClassification", obstacleInImage.left, obstacleInImage.top + 10, 10, ColorRGBA::black, "class: own player");
        }
        else
        {
          obstacleOnField.type = ObstaclesFieldPercept::opponentPlayer;
          DRAW_TEXT("module:JerseyClassifierProvider2020:jerseyClassification", obstacleInImage.left, obstacleInImage.top + 10, 10, ColorRGBA::black, "class: oponent");
        }
      }
      else
      {
...
std::function<bool(int, int)> JerseyClassifierProvider2020::getPixelClassifier(const GameState::Team::Color teamColor, const GameState::Team::Color otherColor, const int maxBrightness) const
{
  ...
  if(teamColor == GameState::Team::Color::gray)
  ...
  else if(teamColor == GameState::Team::Color::black && (otherColor == GameState::Team::Color::gray || otherColor == GameState::Team::Color::white))
    return [this, grayRange](const int x, const int y)
    {
      return theECImage.saturated[y][x] < colorDelimiter && theECImage.grayscaled[y][x] < grayRange.min;
    };
  else if(teamColor == GameState::Team::Color::white && (otherColor == GameState::Team::Color::black || otherColor == GameState::Team::Color::gray))
    return [this, grayRange](const int x, const int y)
    {
  ...
  else if(teamColor == GameState::Team::Color::black)
  ...
  else if(teamColor == GameState::Team::Color::white)
    return [this, grayRange, otherHue](const int x, const int y)
    {
  ...
  else if(otherColor == GameState::Team::Color::white || otherColor == GameState::Team::Color::black || otherColor == GameState::Team::Color::gray)
  {
