/**
 * @file JointPlayAnalyser.cpp
 * @author Philip Reichenberg
 */

#include "JointPlayAnalyser.h"

MAKE_MODULE(JointPlayAnalyser);

JointPlayAnalyser::JointPlayAnalyser()
{
  for(std::size_t joint = 0; joint < Joints::numOfJoints; joint++)
    minMaxValue[joint] = maxRequestChange[joint] = Rangea(0, 0);
}

void JointPlayAnalyser::update(JointPlayData& theJointPlayData)
{
  DEBUG_RESPONSE_ONCE("module:JointPlayAnalyser:reset")
  {
    for(std::size_t joint = 0; joint < Joints::numOfJoints; joint++)
