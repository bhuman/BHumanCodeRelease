...
option(PenaltyTaker)
{
  ...
  for(auto& percept : theObstaclesFieldPercept.obstacles)
  {
    ...
    if(perceptOnField.x() < theFieldDimensions.xPosOpponentGoalArea - 300.f
       || perceptOnField.x() > theFieldDimensions.xPosOpponentGroundLine + 100.f
  ...
  if(theGameState.isPushingFreeKick())
    theSaySkill({.text = "Taking Penalty"});
  ...
  initial_state(initial)
  {
    transition
    {
      if(std::min(timeSincePenaltyShootoutStarted, state_time) > 3000)
      {
        if(kickLeft())
    ...
    action
    {
      theLookLeftAndRightSkill({.maxPan = 20_deg,
                                .tilt = 5.7_deg,
                                .speed = 30_deg});
      theStandSkill();
    }
