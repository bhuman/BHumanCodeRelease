...
#pragma once
#include "Representations/MotionControl/DiveEngineOutput.h"
...
{
  ENUM(Motion,
  {, //all motions defined in diveEngine.cfg TODO
    /**front,
    frontFast,
    back,
    backFast,
...
STREAMABLE(DiveEngineParameters,
{,
...
{,
  ...
  USES(MotionInfo),
  PROVIDES(DiveEngineOutput),
});

class DiveEngine : public DiveEngineBase
{
private:
  ENUM(EngineState,
  {,
  ...
  float kp, //pid factors   - proportionnel - Controleur moteur
        ki, // integrale
        kd, //derivée
...
public:
  ...
  void update(DiveEngineOutput& output);
  ...
  DiveEngine() :
    ...
    gLast(0.f)
  {
    InMapFile stream("diveEngine.cfg");
   ...
   * @param output the DiveEngineOutput object for changing joint stiffness
   */
  void pickMotion(DiveEngineOutput& output);
   ...
   * @param output the DiveEngineOutput object for changing target joint angles
   */
  void setNextJoints(DiveEngineOutput& output);

  /**
  * The method writes a stiffness to the output
  * @param output the DiveEngineOutput object the stiffness should be set to
  * @param stiffness 0-100 percent stiffness
  */
  void setStiffnessAllJoints(DiveEngineOutput& output, int stiffness);
  ...
  */
  ...
  bool verifyUprightTorso(DiveEngineOutput& output);

  /*
  * This method writes a diveMotion into diveEngineDummy.mof for better debugging
  ...
  * @param motion motion to be written into specialAction diveEngineDummy.mof
  */
  void generateMofOfMotion(Mof::Motion motionName);

  /*
  * This method writes a diveEngineDummy.mof file into diveEngine.cfg, use it only offline in simulation!!!
  ...
  * @param motion to be overwritten by specialAction diveEngineDummy.mof
  */
