...
* This file implements a module that point with arms at specified points.
...
void PointAtEngine::update(Arms::Arm arm, PointAtEngineOutput& pointAtEngineOutput)
{
  ...
  if(!theTorsoMatrix.isValid
    ...
    || (theRobotModel.limbs[Limbs::combine(arm, Limbs::shoulder)].inverse() * point).squaredNorm() < sqr(theRobotDimensions.lowerArmLength + theRobotDimensions.upperArmLength)//not for anouth away from shoulder
  ...
  pointAtEngineOutput.angles[Joints::combine(arm, Joints::wristYaw)] = 0;
