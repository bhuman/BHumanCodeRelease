/**
 * @file Tools/Communication/RemoteControlChannel.cpp
 *
 * The file implements a class for the communication between a PC and
...
void RemoteControlChannel::startLocal(int port, unsigned localId)
{
  ASSERT(!this->port);
  this->port = port;
...
void RemoteControlChannel::start(int port, const std::string& target)
{
  ASSERT(!this->port);
  this->port = port;
  this->target = target;
...
void RemoteControlChannel::send(const char* data, int length)
{
  if(port)
    socket.write(data, length);
}

int RemoteControlChannel::receive(char* data, int length)
{
  if(!port)
