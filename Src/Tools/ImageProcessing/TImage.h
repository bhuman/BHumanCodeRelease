...
public:
  TImage() : width(0), height(0) {}
  TImage(const unsigned int width, const unsigned int height, const unsigned int padding = 0)
  {
  ...
  TImage& operator=(const TImage<Pixel>& other)
  {
  ...
  static std::string expandImageFileName(const std::string& fileName, int imageNumber)
  {
    ...
    const size_t lastDirectorySeparatorIndex = (int)name.find_last_of("\\/");
    ...
    const size_t lastDirectorySeparatorIndex2 = (int)name.find_last_of("\\/");
                   ...
                   const int imageNumber = -1, const ExportMode mode = raw) const
  {
    ...
    for(int y = std::max(top, 0); y < static_cast<int>(height) && y < bottom; ++y)
    {
      ...
      size_t xStart = std::max(left, 0) % Pixel::numPixel();
...
using GrayscaledImage = TImage<PixelTypes::GrayscaledPixel>;
using YUYVImage = TImage<PixelTypes::YUYVPixel>;
using BGRAPixel = TImage<PixelTypes::BGRAPixel>;

#ifndef TARGET_ROBOT
template<> inline bool TImage<PixelTypes::GrayscaledPixel>::exportImage(const std::string& fileName, const int imageNumber, const ExportMode mode) const
{
  const std::string name = expandImageFileName(fileName, imageNumber);
  QImage::Format format = (mode == grayscale) ? QImage::Format_Grayscale8 : QImage::Format_RGB888;
  int numChannels = (mode == grayscale) ? 1 : 3;
...
template<typename Pixel>
class TImageWrapper : public TImage<Pixel>
{
public:
  TImageWrapper(const unsigned int width, const unsigned int height, Pixel* theImage)
  {
    this->image = theImage;
  ...
  {
    FAIL("TImageWrapper does not manage its imagestorage itself.");
  }
};
