...
void CodedContour::evaluateX16Y16(signed short responseBin[16][16], const CNSImage& img, int x, int y) const
{
  responseX16Y16RUsingSSE3(&img(x + referenceX, y + referenceY), img.width * sizeof(CNSResponse), &responseBin[0][0], *this);
}

void CodedContour::evaluateX8Y8(signed short responseBin[8][8], const CNSImage& img, int x, int y) const
{
  ...
  for(int y = -r; y <= +r; y++)
    for(int x = -r; x <= +r; x++)
    {
      ...
      if(dMin <= d && d < dMax) // The point belongs to the circle
        contour.push_back(codeContourPointWithAngle(x, y, atan2((double) y, (double) x), n));
    }
  return contour;
}
...
{
  assert(x0 <= x1);
  int x0i = (int) x0, x1i = (int) x1;
  double y = y0 + (x0i - x0) * a;
  for(int x = x0i; x <= x1i; x++)
  {
    contour.push_back(ContourPoint(x, (int) round(y), angle));
    y += a;
  }
}
...
void lineX(Contour& contour, double y0, double y1, double a, double x0, double angle)
{
  ...
  {
    contour.push_back(ContourPoint((int) round(x), y, angle));
    x += a;
  }
}
...
void line(Contour& contour, double x0, double y0, double x1, double y1, bool sparse)
{
  if(sparse)
  {
    ...
    contour.push_back(ContourPoint((int) round((x0 + x1) / 2), (int) round((y0 + y1) / 2), angle));
  }
  else
