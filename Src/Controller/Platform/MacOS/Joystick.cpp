             ...
             elemType == kIOHIDElementTypeInput_ScanCodes)
          {
            if(IOHIDElementGetUsagePage(elem) == kHIDPage_GenericDesktop)
              ...
              case kHIDUsage_GD_Rz:
              {
                ...
                axisIds[axis] = elem;
                axisMin[axis] = IOHIDElementGetLogicalMin(elem);
                axisMax[axis] = IOHIDElementGetLogicalMax(elem);
                break;
              }
...
bool Joystick::update()
{
  ...
  for(int i = 0; i < numOfAxes; ++i)
    if(axisIds[i])
    {
      ...
      axisState[i] = IOHIDValueGetIntegerValue(valueRef);
    }
  ...
  if(hatId)
  {
    ...
    int dir = IOHIDValueGetIntegerValue(valueRef);
