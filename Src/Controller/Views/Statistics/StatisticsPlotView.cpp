/**
 * @file Controller/Views/Statistics/StatisticsPlotView.cpp
 *
 * Implementation of class StatisticsPlotView.
 * Based on implementation of @file Controller/Views/PlotView.cpp
 *
...
#include "StatisticsPlotView.h"
#include <algorithm>

StatisticsPlotWidget::StatisticsPlotWidget(StatisticsPlotView& plotView, StatisticsPlotWidget*& plotWidget) :
...
bool StatisticsPlotWidget::needsRepaint() const
{
  for(const RobotConsole::Layer& layer : plotView.statistics.plotViews[plotView.name])
    if(plotView.statistics.plots[layer.layer].timeStamp > lastTimeStamp)
      return true;
  return false;
}

void StatisticsPlotWidget::paintEvent(QPaintEvent* event)
{
  painter.begin(this);
  if(antialiasing)
...
void StatisticsPlotWidget::paint(QPainter& painter)
{
  ...
  // draw axis lables
  {
    ...
    // x
    {
      ...
      {
        sprintf(buf, "%g", (plotView.plotSize - pos) * plotView.xScale);
  ...
  int legendWidth = 0;
  const std::list<RobotConsole::Layer>& plotList = plotView.statistics.plotViews[plotView.name];
  for(const RobotConsole::Layer& layer : plotList)
  {
    const std::list<float>& list = plotView.statistics.plots[layer.layer].points;
    ...
    unsigned int timeStamp = plotView.statistics.plots[layer.layer].timeStamp;
...
void StatisticsPlotWidget::determineMinMaxValue()
{
  bool started = false;
  {
    const std::list<RobotConsole::Layer>& plotList = plotView.statistics.plotViews[plotView.name];
...
void StatisticsPlotWidget::toggleDrawUnits()
{
  drawUnits = !drawUnits;
  QWidget::update();
}

void StatisticsPlotWidget::toggleDrawLegend()
{
  drawLegend = !drawLegend;
  QWidget::update();
}

void StatisticsPlotWidget::toggleAntialiasing()
{
  antialiasing = !antialiasing;
  QWidget::update();
}

void StatisticsPlotWidget::exportAsGnuplot()
{
...
QMenu* StatisticsPlotWidget::createUserMenu() const
{
...
void StatisticsPlotWidget::update()
{
  if(needsRepaint())
    QWidget::update();
}

StatisticsPlotView::StatisticsPlotView(const QString& fullName, Statistics& statistics, const std::string& name,
                                       unsigned int plotSize, float minValue, float maxValue,
                                       const std::string& yUnit, const std::string& xUnit, float xScale) :
  fullName(fullName), icon(":/Icons/tag_green.png"), statistics(statistics), name(name), plotSize(plotSize),
...
StatisticsPlotView::~StatisticsPlotView()
{
  delete[] points;
}

void StatisticsPlotView::setParameters(unsigned int plotSize, float minValue, float maxValue, const std::string& yUnit, const std::string& xUnit, float xScale)
{
  delete[] points;
  points = new QPointF[plotSize];
  for(unsigned int i = 0; i < plotSize; ++i)
...
SimRobot::Widget* StatisticsPlotView::createWidget()
{
  ASSERT(!plotWidget);
  plotWidget = new StatisticsPlotWidget(*this, plotWidget);
  return plotWidget;
}
