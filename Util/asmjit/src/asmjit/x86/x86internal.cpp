...
ASMJIT_FAVOR_SIZE Error X86Internal::initFrameLayout(FuncFrameLayout& layout, const FuncDetail& func, const FuncFrameInfo& ffi) noexcept {
  ...
  layout._gpStackSize  = static_cast<uint16_t>(Utils::bitCount(layout.getSavedRegs(X86Reg::kKindGp )) * gpSize);
  layout._vecStackSize = static_cast<uint16_t>(Utils::bitCount(layout.getSavedRegs(X86Reg::kKindVec)) * 16 +
