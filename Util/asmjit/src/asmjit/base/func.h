...
# error "[asmjit] Couldn't determine the target's calling convention."
  ...
  ASMJIT_INLINE void setFlags(uint32_t flag) noexcept { _flags = static_cast<uint8_t>(flag); };
  //! Add calling convention flags, see \ref Flags.
...
public:
  ...
  ASMJIT_INLINE void setRet(uint32_t retType) noexcept { _ret = static_cast<uint8_t>(retType); }
  ...
  ASMJIT_INLINE void setArg(uint32_t i, uint32_t type) noexcept {
    ASMJIT_ASSERT(i < _argCount);
    _builderArgList[i] = static_cast<uint8_t>(type);
  }
...
struct FuncFrameInfo {
  ...
  ASMJIT_INLINE void setStackArgsRegId(uint32_t regId) { _stackArgsRegId = static_cast<uint8_t>(regId); }
