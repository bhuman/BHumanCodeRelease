...
class _Node:
    def __init__(self, node: bhlogs.Record) -> None:
        self.option: str = node.option
        self.depth: int = node.depth
...
class BehaviorOscillation(Statistic):
    ...
    def _update(self, graph: list[_Node], frame_idx: int):
    ...
    def update(self, frame: bhlogs.Frame, frame_idx: int):
        if frame.thread != "Cognition":
            return
        if "ActivationGraph" not in frame:
            return
        activation_graph = frame["ActivationGraph"]
        if not hasattr(activation_graph, "graph"):
            return

        graph: list[_Node] = [_Node(node) for node in activation_graph.graph]
