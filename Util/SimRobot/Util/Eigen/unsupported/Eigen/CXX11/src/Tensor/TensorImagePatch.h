...
struct TensorEvaluator<const TensorImagePatchOp<Rows, Cols, ArgType>, Device>
{
      ...
      : m_impl(op.expression(), device)
  {
    ...
    if (op.padding_explicit()) {
      m_outputRows = static_cast<int>(numext::ceil((m_input_rows_eff + op.padding_top() + op.padding_bottom() - m_patch_rows_eff + 1.f) / static_cast<float>(m_row_strides)));
      m_outputCols = static_cast<int>(numext::ceil((m_input_cols_eff + op.padding_left() + op.padding_right() - m_patch_cols_eff + 1.f) / static_cast<float>(m_col_strides)));
      ...
      switch (op.padding_type()) {
        case PADDING_VALID:
          m_outputRows = static_cast<int>(numext::ceil((m_input_rows_eff - m_patch_rows_eff + 1.f) / static_cast<float>(m_row_strides)));
          m_outputCols = static_cast<int>(numext::ceil((m_input_cols_eff - m_patch_cols_eff + 1.f) / static_cast<float>(m_col_strides)));
          ...
          break;
        case PADDING_SAME:
          m_outputRows = static_cast<int>(numext::ceil(m_input_rows_eff / static_cast<float>(m_row_strides)));
          m_outputCols = static_cast<int>(numext::ceil(m_input_cols_eff / static_cast<float>(m_col_strides)));
          // Calculate the padding
